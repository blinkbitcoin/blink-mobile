...
export const RecoveryBackupScreen: React.FC = () => {
  ...
  // Memoized: toLocaleString can be expensive with large ICU datasets on
  // some Android devices, and this re-renders on every state change.
  const formatWhen = useMemo(
    () => (unixMs: number) => new Date(unixMs).toLocaleString(locale),
    [locale],
  )
