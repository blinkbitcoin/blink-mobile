...
/**
 * How the flow came by its target wallet. It decides what the completion step may write
 * onto that account: seeding the custodial settings and marking a backup are right for a
 * wallet this flow made and wrong for one the user already owns.
 *
 * Absent on records saved before the field existed, and on those it means "provisioned" —
 * the only way a target could be obtained then.
 */
export const MigrationTargetOrigin = {
  /** Generated by this flow. Empty, unbacked, and disposable until the migration commits. */
  Provisioned: "provisioned",
  /** Derived here from a phrase the user typed. New to this device, but theirs already. */
  Restored: "restored",
  /** The phrase named an account this device already held, with its own settings in place. */
  Adopted: "adopted",
} as const

export type MigrationTargetOrigin =
  (typeof MigrationTargetOrigin)[keyof typeof MigrationTargetOrigin]
...
export type StoredCheckpoint = {
  ...
  sparkInvoice?: string
  /** See MigrationTargetOrigin. Absent means provisioned. */
...
export const validateStoredCheckpoint = (raw: unknown): StoredCheckpoint | null => {
  ...
  /** Advisory in the same way: an unrecognised origin drops to undefined, which the readers
   *  treat as provisioned — the behaviour every record written before the field had. */
  const hasUsableTargetOrigin = Object.values(MigrationTargetOrigin).includes(
    targetOrigin as MigrationTargetOrigin,
  )
  ...
  return {
    ...
    targetOrigin: hasUsableTargetOrigin ? targetOrigin : undefined,
  }
}
...
export const mergeCheckpoint = (
  ...
  return {
    ...
    /** Travels with the accountId it describes: inheriting one owner's origin onto another
     *  owner's wallet would let the completion step write by the wrong rule. */
    targetOrigin:
      update.targetOrigin ?? (hasSameOwner ? existing?.targetOrigin : undefined),
  }
}
