...
export const useMigrationAccount = () => {
  ...
  const ensureAccount = useCallback(async (): Promise<string | null> => {
    ...
    /** Kept for the catch: the checkpoint write reports its failure kind rather than
     *  throwing it, while the pending-account write throws the storage error itself. */
    let checkpointFailure: StorageFailure | null = null
    try {
      ...
      const failure = checkpointFailure ?? classifyStorageFailure(err)
