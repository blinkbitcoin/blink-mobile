...
export const useMigrationBalancesPreview = ({
  ...
  /** Every source gates the screen: the balances feed the current Dollar Balance, the
   *  preview feeds every bitcoin figure, and the checkpoint decides how the new Dollar
   *  row reads; none may render before it is known. */
  const hasPreview = preview !== null
  const isVerdictPending = isDollarRegionPending || isProvisionedAccountLoading
  const isLoading = isPreviewLoading || areBalancesLoading || isVerdictPending
  const isReady = areBalancesReady && hasPreview && !isVerdictPending
