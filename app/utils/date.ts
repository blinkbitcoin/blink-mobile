 ...
 * matching the transaction list and detail screens; `timezone` is passed by tests to pin
 * a zone and by the investment agreement, which dates itself in its host's zone. Both
 * hand over a literal, so a malformed string never reaches `toLocaleTimeString`, which
 * would throw a RangeError: give this the guard `formatDayAndMonth` carries if a backend
 * zone ever arrives here. Each half borrows the locale whose shape it needs, `en-CA` for
 * the `YYYY-MM-DD` order and `en-GB` for the 24 hour clock, and joining them here keeps
 * the result stable whatever separator a locale would pick; `en-GB` already defaults to
 * `h23`, so `hourCycle` is not load-bearing and only guards a device ICU that would
 * resolve it otherwise and render midnight as `24`.
 */
