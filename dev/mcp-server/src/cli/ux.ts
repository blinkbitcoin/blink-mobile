...
async function uxSend(
  destination: string,
  amount: string,
  opts: { wallet?: string; note?: string },
) {
  console.log(`Sending ${amount} to ${destination}...`)

  if (!(await goHome())) fail("Could not navigate to home first")
  if (!(await tapAndWait("Send", "Username, invoice, or address")))
    fail("Could not open Send screen")
  ...
  if (!(await tapAndWait("Next", "Amount Input Button")))
    fail("Could not proceed to amount screen")
  ...
  if (opts.wallet) {
    ...
    await pause(300)
    const walletId =
      opts.wallet.toLowerCase() === "usd" ? "stablesats-balance" : "bitcoin-balance"
...
async function uxLogin(opts: { phone?: string; code?: string; country?: string }) {
  ...
  if (!phone || !code)
    fail("Phone and code required - set in config.yaml or pass via flags")
  ...
  if (!(await waitForElement("telephoneNumber", TIMEOUTS.short)))
    fail("Could not reach SMS login screen")
  ...
  if (!(await hasElement("oneTimeCode")) && !(await hasElement("home-screen")))
    fail("Code screen not shown")
  ...
  const result = await waitForAny(
    ["home-screen", "Geetest", "icon-warning", "permission_allow_button"],
    TIMEOUTS.medium,
  )
...
export function registerUxCommands(program: Command) {
  ...
  ux.command("home").description("Navigate to home screen").action(uxHome)

  ux.command("settings").description("Open settings menu").action(uxSettings)
