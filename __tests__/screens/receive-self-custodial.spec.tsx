...
describe("ReceiveScreen — self-custodial", () => {
  ...
  describe("New address action on the on-chain page", () => {
    const renderAndOpenOnchainPage = async () => {
      render(
        <ContextForScreen>
          <ReceiveScreen />
        </ContextForScreen>,
    ...
    it("is hidden on the lightning page", async () => {
      render(
        <ContextForScreen>
          <ReceiveScreen />
        </ContextForScreen>,
    ...
    it("renders on the on-chain page", async () => {
      await renderAndOpenOnchainPage()

      expect(screen.getByText(LL.ReceiveScreen.newAddress())).toBeTruthy()
    })

    it("asks the SDK for a fresh address when pressed", async () => {
      await renderAndOpenOnchainPage()

      mockReceiveOnchain.mockClear()
      mockReceiveOnchain.mockResolvedValue({ address: "bc1qscrotated..." })

      await act(async () => {
        fireEvent.press(screen.getByText(LL.ReceiveScreen.newAddress()))
        await flushAsync()
      })

      expect(mockReceiveOnchain).toHaveBeenCalledWith({ newAddress: true })
      await waitFor(() => {
        expect(screen.getByText("bc1qscrotated...")).toBeTruthy()
      })
    })
  })
})
