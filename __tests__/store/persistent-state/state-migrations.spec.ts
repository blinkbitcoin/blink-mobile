...
describe("state-migrations schema 10", () => {
  ...
  it("carries txLastSeenByAccountId through the v21 step untouched", async () => {
    const state21 = {
      schemaVersion: 21,
      galoyInstance: { id: "Main" },
      galoyAuthToken: "token",
  ...
  it("leaves cardInvestmentByAccountId undefined when migrating from v21", async () => {
    const state21 = {
      schemaVersion: 21,
      galoyInstance: { id: "Main" },
      galoyAuthToken: "token",
      activeAccountId: "self-custodial-id-1",
      txLastSeenByAccountId: { "self-custodial-id-1": { btcId: "btc-1", usdId: "" } },
    }

    const result = await migrateAndGetPersistentState(state21)

    expect(result.cardInvestmentByAccountId).toBeUndefined()
  })
