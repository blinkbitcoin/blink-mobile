...
describe("state-migrations schema 10", () => {
  ...
  it("preserves completedQuizIdsByAccountId when migrating a v16 state to current", async () => {
    const state16 = {
      schemaVersion: 16,
      galoyInstance: { id: "Main" },
      galoyAuthToken: "token",
  ...
  it("v17 identity migration preserves the stored account modes untouched", async () => {
    const state17 = {
      schemaVersion: 17,
      galoyInstance: { id: "Main" },
      galoyAuthToken: "token",
    ...
    const result = await migrateAndGetPersistentState(state17)
  ...
  it("default state has the current schema version", () => {
    expect(defaultPersistentState.schemaVersion).toBe(17)
