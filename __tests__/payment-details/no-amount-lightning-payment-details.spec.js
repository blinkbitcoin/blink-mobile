...
import { testAmount, btcSendingWalletDescriptor, convertMoneyAmountMock, createGetFeeMocks, createSendPaymentMocks, expectCannotGetFee, expectCannotSendPayment, expectDestinationSpecifiedMemoCannotSetMemo, usdSendingWalletDescriptor, zeroAmount, } from "./helpers";
var defaultParams = {
...
describe("no amount lightning payment details", function () {
    var createNoAmountLightningPaymentDetails = PaymentDetails.createNoAmountLightningPaymentDetails;
    it("properly sets fields with all arguments provided", function () {
        var paymentDetails = createNoAmountLightningPaymentDetails(defaultParams);
    ...
    describe("sending from a btc wallet", function () {
        var btcSendingWalletParams = __assign(__assign({}, defaultParams), { unitOfAccountAmount: testAmount, sendingWalletDescriptor: btcSendingWalletDescriptor });
        var paymentDetails = createNoAmountLightningPaymentDetails(btcSendingWalletParams);
        var settlementAmount = defaultParams.convertMoneyAmount(testAmount, btcSendingWalletDescriptor.currency);
    ...
    describe("sending from a usd wallet", function () {
        var usdSendingWalletParams = __assign(__assign({}, defaultParams), { unitOfAccountAmount: testAmount, sendingWalletDescriptor: usdSendingWalletDescriptor });
        var settlementAmount = defaultParams.convertMoneyAmount(testAmount, usdSendingWalletDescriptor.currency);
        var paymentDetails = createNoAmountLightningPaymentDetails(usdSendingWalletParams);
        it("uses the correct fee mutations and args", function () { return __awaiter(void 0, void 0, void 0, function () {
            var feeParamsMocks, _a;
            return __generator(this, function (_b) {
                switch (_b.label) {
    ...
    it("cannot calculate fee or send payment with zero amount", function () {
        var params = __assign(__assign({}, defaultParams), { unitOfAccountAmount: zeroAmount });
        var paymentDetails = createNoAmountLightningPaymentDetails(params);
        expectCannotGetFee(paymentDetails);
        expectCannotSendPayment(paymentDetails);
    });
    it("cannot set memo if memo is provided", function () {
        var paramsWithMemo = __assign(__assign({}, defaultParams), { destinationSpecifiedMemo: "sender memo" });
        var paymentDetails = createNoAmountLightningPaymentDetails(paramsWithMemo);
